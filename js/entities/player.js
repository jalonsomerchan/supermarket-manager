...
export class Player {
  ...
  interactionTarget(game) {
    ...
    const shopRect = { x: config.map.entrance.x, y: config.map.entrance.y, w: 2, h: 1 };
    if (nearbyRect(this, shopRect, config.tile)) return { type: "shop", label: state.phase === "closed" ? "Abrir tienda" : "Cerrar tienda", rect: shopRect };
